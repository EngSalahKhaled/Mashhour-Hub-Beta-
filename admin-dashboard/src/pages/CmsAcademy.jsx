...
// ─── Modal ────────────────────────────────────────────────────────────────────
function AcademyModal({ item, onClose, onSave }) {
  const [form, setForm] = useState(item || { 
    title_en: '', title_ar: '', slug: '', 
    excerpt_en: '', excerpt_ar: '', 
    content_en: '', content_ar: '', 
    thumbnail: '', video_url: '', price: 0, instructor: '', status: 'draft' 
  });
          ...
          >
            <div>
              ...
              <h2 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>
                {item?.id ? 'Edit Course' : 'Add New Course'}
              </h2>
            </div>
            <button
              onClick={onClose}
            ...
            >
              <div className="p-8 space-y-6 flex-1 text-[var(--text-primary)]">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>Instructor</label>
                    <input className="input-field" value={form.instructor} onChange={set('instructor')} placeholder="Instructor Name" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className="input-field" value={form.slug} onChange={set('slug')} placeholder="e.g. digital-marketing-101" />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>Price ($ or KWD)</label>
                    <input type="number" className="input-field" value={form.price} onChange={set('price')} placeholder="0 for Free" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>Video Trailer URL (Optional)</label>
                    <input className="input-field" value={form.video_url} onChange={set('video_url')} placeholder="YouTube / Vimeo link" />
                  </div>
                </div>

                {/* English Content */}
                <div className="p-5 rounded-xl border border-[rgba(99,179,237,0.1)] bg-[rgba(255,255,255,0.01)] space-y-4">
                  <h3 className="font-bold text-lg text-cyan-400">🇬🇧 English Details</h3>
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>Course Title</label>
                  ...
                  <div>
                    ...
                    <textarea className="input-field resize-none" rows={2} value={form.excerpt_en} onChange={set('excerpt_en')} placeholder="Short intro..." />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>Full Description</label>
                ...
                <div className="p-5 rounded-xl border border-[rgba(99,179,237,0.1)] bg-[rgba(255,255,255,0.01)] space-y-4" dir="rtl">
                  <h3 className="font-bold text-lg text-emerald-400">🇸🇦 التفاصيل بالعربية</h3>
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>عنوان الدورة</label>
                    <input className="input-field" value={form.title_ar} onChange={set('title_ar')} placeholder="العنوان بالعربي" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>وصف قصير</label>
                    <textarea className="input-field resize-none" rows={2} value={form.excerpt_ar} onChange={set('excerpt_ar')} placeholder="مقدمة قصيرة..." />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>الوصف الكامل</label>
              ...
              >
                ...
                <button onClick={handleSave} disabled={saving} className="btn-primary !px-8 !py-3 !text-base">
                  {saving
                    ...
                    : item?.id ? '✓ Save Changes' : '+ Publish Course'
                  }
                </button>
              </div>
            </div>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function CmsAcademy() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Academy</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>Manage your educational courses and lessons</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 rounded-2xl" style={{ border: '1px dashed rgba(99,179,237,0.2)' }}>
          <p style={{ color: 'var(--text-muted)' }}>No courses found. Create your first one!</p>
        </div>
      ) : (
